...
func ManualUpdateBeatmapSet(id int) {
	...
	var data bodyStruct.BeatmapSetsIN
...
func getUpdatedMapRanked() {
	...
	var data bodyStruct.BeatmapsetsSearch
...
func getGraveyardMap() {
	...
	url := ""
	lu := &config.Config.Osu.BeatmapUpdate.GraveyardAsc.LastUpdate
	id := &config.Config.Osu.BeatmapUpdate.GraveyardAsc.Id
	if *lu+*id != "" {
...
func getUpdatedMapDesc() {
	...
	config.Config.Osu.BeatmapUpdate.UpdatedDesc.LastUpdate = *data.Cursor.LastUpdate
	config.Config.Osu.BeatmapUpdate.UpdatedDesc.Id = *data.Cursor.Id
...
func getUpdatedMapAsc() {
	...
	url := ""
	lu := &config.Config.Osu.BeatmapUpdate.UpdatedAsc.LastUpdate
	id := &config.Config.Osu.BeatmapUpdate.UpdatedAsc.Id
	if *lu+*id != "" {
...
func updateMapset(s *bodyStruct.BeatmapSetsIN) {
	...
	r := *s.Ratings
	_, err := db.Maria.Exec(UpsertBeatmapSet, s.Id, s.Artist, s.ArtistUnicode, s.Creator, s.FavouriteCount,
...
func upsertMap(m bodyStruct.BeatmapIN, ch chan struct{}) {
	_, err := db.Maria.Exec(UpsertBeatmap, m.Id, m.BeatmapsetId, m.Mode, m.ModeInt, m.Status, m.Ranked, m.TotalLength, m.MaxCombo, m.DifficultyRating, m.Version,
...
func updateSearchBeatmaps(data *[]bodyStruct.BeatmapSetsIN) (err error) {
	if data == nil {
		return
	}
	if len(*data) < 1 {
